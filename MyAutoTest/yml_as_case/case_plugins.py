...
在<Session>之上还有一个<Config>包，用于初始化运行环境的各种配置。
各包存在层级关系，由上层包可以构建下层包,
pytest会自动创建 Config Session Package层级的包

整体思路：
1.if判断筛选addopts路径下符合条件的yaml文件
2.继承pytest的Module，动态创建Module实例对象
3.由types动态生成一个python的module实例对象
4.为module对象添加一个解析yaml，执行测试的function，这个function就被视为一个用例
5.通过猴子补丁将Module对象的_getobj方法重写，获取module对象
6.return 返回Module对象
整体结构：Module-module-function
"""
...
#         runner.run()  # 实际是给 module添加了一个同名的function，function即视为用例，在这个function里解析yaml，执行测试
...
#         # 模块变量渲染
#         self.context.update(__builtins__)  # 加载内置函数, 实际update的是__builtins__.__dict__
#         self.context.update(debug.__dict__)  # 自定义函数对象
...
class YamlFile(pytest.File):
    def collect(self):
        ...
        for name, spec in sorted(raw.items()):
            if name == "case_config":
                case_name = raw.get("case_config").get("name", "")
            elif name == "case_steps":
                for index, step in enumerate(spec):
                    step_name = step.get("name", f"testcase{index + 1}")
                    yield YamlItem.from_parent(self, spec=step, name=step_name)
